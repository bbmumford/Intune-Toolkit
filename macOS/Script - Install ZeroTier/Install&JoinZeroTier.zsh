...
if [[ $(uname -m) == 'arm64' ]]; then
    ...
    weburl="https://download.zerotier.com/dist/ZeroTierOne.pkg" 
    else
    # This is x64 URL
    weburl="https://download.zerotier.com/dist/ZeroTierOne.pkg"   
fi

appname="ZeroTier"                                                      # The name of our App deployment script (also used for Octory monitor)
app="ZeroTier.app"                                                      # The actual name of our App once installed
...
processpath="$appdir/$app/Contents/MacOS/ZeroTier"                      # The process name of the App we are installing
...
# Define global variables for ZeroTier
zerotierNetworkID="<YourZeroTierNetworkID>"
zerotierAuthToken="<YourZeroTierAuthToken>"
...
# Function to add device to ZeroTier network
addDeviceToZeroTier() {
    zerotier-cli join $zerotierNetworkID
    if [ $? -eq 0 ]; then
        echo "$(date) | Successfully joined ZeroTier network $zerotierNetworkID"
    else
        echo "$(date) | Failed to join ZeroTier network $zerotierNetworkID"
        exit 1
    fi
}

# Function to authorize device using ZeroTier API
authorizeDevice() {
    local deviceID=$(zerotier-cli info | grep "200 info" | awk '{print $3}')
    if [ -z "$deviceID" ]; then
        echo "$(date) | Failed to get ZeroTier device ID"
        exit 1
    fi

    local apiURL="https://my.zerotier.com/api/network/$zerotierNetworkID/member/$deviceID"
    local response=$(curl -s -X POST -H "Authorization: Bearer $zerotierAuthToken" -H "Content-Type: application/json" -d '{"authorized":true}' $apiURL)

    if [[ "$response" == *"authorized"* ]]; then
        echo "$(date) | Successfully authorized device $deviceID on ZeroTier network $zerotierNetworkID"
    else
        echo "$(date) | Failed to authorize device $deviceID on ZeroTier network $zerotierNetworkID"
        exit 1
    fi
}
...
# Join ZeroTier network and authorize device
addDeviceToZeroTier
authorizeDevice